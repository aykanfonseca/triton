...
class Tag extends PureComponent {
	...
	tagMaker = () => {
			...
			if (doesContain !== -1) {
				if (
					this.props.prerequisites
						.toLowerCase()
						.search(new RegExp('\\b' + course.code.toLowerCase() + '\\b')) !== -1
				) {
		...
		this.check.forEach((course) => {
			this.tester = this.tester.replace(
				new RegExp(course.code, 'ig'),
				'<span title="Click to take a look!">' + course.code + '</span>'
			);
		});
	};
	...
	render() {
		return (
			<p
				style={{ marginTop: this.tester === 'None' ? '1px' : '7px' }}
				ref={this.recomposer}
				className="tagged_class"
				dangerouslySetInnerHTML={{ __html: this.tester }}
			/>
		);
	}
}
