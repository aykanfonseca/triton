...
export default class QuarterSwitcher extends PureComponent {
	...
	render() {
		const { selectedQuarter, quarters, theme } = this.props;
		const { quarter, showSmall } = this.state;

		if (selectedQuarter !== '') {
			return (
				<select
					title="Select to change quarter"
					className={'quarter' + theme}
					value={quarter}
				...
				>
					...
					{quarters.map((quart) => (
						<option key={quart.abbreviation}>{showSmall ? quart.abbreviation : quart.value}</option>
					))}
					{/* </optgroup> */}
				</select>
			);
		...
		return (
			<div title="Select to change quarter" className={'quarter' + theme}>
				<div className={'quarter' + theme + '-loading'} />
			</div>
		);
	}
}
